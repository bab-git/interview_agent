"""Run a live HTTP smoke test against a running orchestration-prototype server."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run a live smoke test against the orchestration prototype API.")
    ...
    with httpx.Client(base_url=args.base_url, timeout=30.0) as client:
        ...
        start = client.post("/api/interviews", json={"skill": "Communication", "candidate_name": "Smoke Test Session"})
        ...
        if conversation["status"] != "completed":
            raise AssertionError(f"Session did not complete: {conversation['status']}")
        ...
        print(f"Session ID: {interview_id}")
