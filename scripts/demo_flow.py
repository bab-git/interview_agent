"""Run an end-to-end in-process demo of the core reviewer workflow."""
...
def load_client() -> TestClient:
    """Create a demo-scoped TestClient with isolated data and mock LLM settings."""
    ...
    os.environ["DEMO_RESET_ON_START"] = "false"
    os.environ["PROMPT_ADMIN_ENABLED"] = "true"
...
def main() -> None:
    """Execute the core workflow from session start to prompt-version change."""
    with load_client() as client:
        ...
        start = client.post(
            "/api/interviews",
            json={"skill": "Problem Solving", "candidate_name": "Trace Review Demo"},
        )
        session = start.json()
        interview_id = session["id"]
        pretty_print("Session Started", session)
        ...
        pretty_print("Retrieved Session", retrieved.json())
        ...
        feedback = client.post(
            ...
            json={
                ...
                "notes": "The clarification usefully pushed for specifics and made the trace easier to review.",
            },
        )
        ...
        second = client.post(
            "/api/interviews",
            json={"skill": "Problem Solving", "candidate_name": "Prompt Version Review"},
        )
        pretty_print("New Session Uses Updated Prompt", second.json())
